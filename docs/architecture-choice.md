...
- **Couverture** : mesurée sur `@cmv/shared` (tests unitaires), sur `apps/api` (les e2e, instrumentés en v8 — ils lancent Nest dans le process du worker) et sur `apps/web` (Vitest + jsdom + Testing Library, `vitest.config.ts` fusionné avec `vite.config.ts` pour ne pas dupliquer les alias). Le périmètre web est **tout `src/`**, écrans compris : y soustraire les composants les mettrait hors de vue durablement. Seul `apps/mobile` reste hors mesure (#59). Toute exclusion de couverture est **symétrique** — la même ligne dans le `coverage.exclude` de Vitest et dans `sonar.coverage.exclusions` —, un fichier absent de tout lcov valant 0 % et non « non mesuré ».
