...
### Médias
- Photos/vidéos/audio → **object storage** (S3-compatible) via `@aws-sdk/client-s3` + **URLs signées**. Jamais le binaire en BDD. Compression côté client. Limites : vidéo **180 s / 720p / 1 Go**, **10 vidéos + 20 photos (100 Mo) + 20 notes vocales (5 min / 100 Mo)** par débrief (comptes relevés en [#156](https://github.com/Cimavia/cimavia/issues/156) : c'est le nombre, non la taille, qui gênait un athlète débriefant dix voies). Les valeurs vivent dans `@cmv/shared` (`MAX_FEEDBACK_*`) et sont **interpolées** dans les messages de refus — jamais réécrites en dur, sous peine de mentir au premier ajustement.
- Backend abstrait par `infra/storage/StorageService` : endpoint/credentials par variables `S3_*` → **SILO** (fork maintenu de MinIO) en dev local et sur le NAS (docker-compose), **Scaleway** en prod, **Cellar** (Clever Cloud) en v1.0, sans changer le code. Path-style (`S3_FORCE_PATH_STYLE`) pour SILO, virtual-hosted pour Scaleway.
...
- CI GitHub Actions (`ci.yml`, trois jobs, tous **requis** pour merger) : `Lint + Typecheck + Test` (`biome ci`, `turbo typecheck test`, `check:i18n`) · `E2E (isolation multi-tenant)` (les e2e d'isolation contre un Postgres et un SILO jetables) · `SonarCloud Analysis` (les 3 couches, `sonar-project.properties`). Ces libellés sont les noms de **jobs** : les rulesets les exigent sous ce nom exact, en renommer un décroche la porte. **Sentry** sur les 3 couches.
- **Couverture** : mesurée sur les **quatre** paquets — `@cmv/shared` (unitaires), `apps/api` (les e2e, instrumentés en v8 : ils lancent Nest dans le process du worker), `apps/web` et `apps/mobile` (Vitest + jsdom + Testing Library). Le mobile reste sur **Vitest** et non `jest-expo` : ce que le préréglage Expo apporte est le rendu d'un arbre natif, que les tests de logique ne traversent pas. Le périmètre est **tout le code** de chaque paquet, écrans compris — y soustraire les composants les mettrait hors de vue durablement, ce qui est la façon dont la dette Q-1 est née. Corollaire à ne pas mal lire : une décision produit (un ordre, une dérivation, un choix de cycle) se met dans une **fonction pure** non parce que le JSX échapperait à la mesure — il ne lui échappe pas —, mais parce qu'un test de rendu ne peut l'attaquer qu'à travers ce qui s'affiche. La ligne est couverte des deux côtés ; seule la fonction pure permet d'affirmer ce qu'elle DÉCIDE. Toute exclusion de couverture est **symétrique** : la même ligne dans le `coverage.exclude` de Vitest et dans `sonar.coverage.exclusions`, un fichier absent de tout lcov valant 0 % et non « non mesuré ».
