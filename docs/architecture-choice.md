...
- CI GitHub Actions (`ci.yml`, trois jobs, tous **requis** pour merger) : `Lint + Typecheck + Test` (`biome ci`, `turbo typecheck test`, `check:i18n`) · `E2E (isolation multi-tenant)` (les 168 e2e contre un Postgres et un MinIO jetables) · `SonarCloud Analysis` (les 3 couches, `sonar-project.properties`). Ces libellés sont les noms de **jobs** : les rulesets les exigent sous ce nom exact, en renommer un décroche la porte. **Sentry** sur les 3 couches.
