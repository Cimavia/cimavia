...
- CI GitHub Actions (`ci.yml`, trois jobs, tous **requis** pour merger) : `Lint + Typecheck + Test` (`biome ci`, `turbo typecheck test`, `check:i18n`) · `E2E (isolation multi-tenant)` (les 268 e2e contre un Postgres et un MinIO jetables) · `SonarCloud Analysis` (les 3 couches, `sonar-project.properties`). Ces libellés sont les noms de **jobs** : les rulesets les exigent sous ce nom exact, en renommer un décroche la porte. **Sentry** sur les 3 couches.
- **Couverture** : mesurée sur les **quatre** paquets — `@cmv/shared` (unitaires), `apps/api` (les e2e, instrumentés en v8 : ils lancent Nest dans le process du worker), `apps/web` et `apps/mobile` (Vitest + jsdom + Testing Library). Le mobile reste sur **Vitest** et non `jest-expo` : ce que le préréglage Expo apporte est le rendu d'un arbre natif, que les tests de logique ne traversent pas. Le périmètre est **tout le code** de chaque paquet, écrans compris — y soustraire les composants les mettrait hors de vue durablement, ce qui est la façon dont la dette Q-1 est née. Toute exclusion de couverture est **symétrique** : la même ligne dans le `coverage.exclude` de Vitest et dans `sonar.coverage.exclusions`, un fichier absent de tout lcov valant 0 % et non « non mesuré ».
