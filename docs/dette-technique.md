...
| P3-6 | **Tuiles du dashboard non branchées** : « Débriefs à relire » et « Factures en attente » affichent `—`, marquées `// MOCKED`. | Les données n'existent pas encore ; un `0` mentirait (règle nullable). La structure de l'écran, elle, est posée. | **P4** (débriefs) et **P6** (factures) : `grep -r MOCKED` les liste. | 🟡 |
