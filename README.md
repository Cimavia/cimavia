...
- Médias : object storage privé, **URLs signées** (PUT à l'envoi, GET à la lecture) ; le binaire ne transite jamais par l'API. L'API délivre un **ticket** dont le mode dépend de la seule taille : un PUT unique sous 80 Mo, un **envoi découpé** au-delà (`CreateMultipartUpload` → une URL signée par part de 10 Mio → clôture serveur). Le seuil n'est pas cosmétique — le bord réseau refuse tout corps de plus de 100 Mo (mesuré). Les ETags sont relus côté serveur par `ListParts`, jamais fournis par le client. Voir `docs/dette-technique.md` §« Envoi découpé des médias »
...
- Rappels : outil **privé du coach** (scopé `coachId` seul, 403 pour l'athlète) ; « dû » est **calculé à la lecture** (`isReminderDue`), et un rappel dû remonte dans le centre de notifications comme entrée **non persistée** (`REMINDER_DUE`, id préfixé `reminder:`). Repousser un rappel (`PATCH /reminders/:id`) **efface son `readAt`** dès que l'échéance bouge : `readAt` dit « vu à cette échéance-là », sans quoi le rappel reviendrait déjà lu et son badge ne s'allumerait jamais. L'écran mobile n'est **pas un onglet** : il vit sous `app/reminders/`, atteint depuis la tuile du tableau de bord coach, garde de capacité dans le `_layout`
- Rappels automatiques : générés par un **tick externe** (`POST /internal/reminders/tick`, secret partagé + workflow `schedule`) — un cron in-process ne se déclencherait pas sur du scale-to-zero. Le tick ouvre un **contexte tenant par coach** plutôt que de contourner l'extension Prisma ; l'idempotence vit dans un index unique et dans `pushedAt`, pas dans le code. Un rappel généré porte un `reason` (enum) et **aucune note** : l'API ne fabrique jamais de libellé
